...
public class GetClothingColor {

    static ColorName target;
    /**
    ...
    public static String getSimilarColor(String color, List<String> colorNames) {
        ArrayList<ColorName> availableColors = new ArrayList<>();
        ArrayList<ColorName> colorList = initColorList();

        //convert String color names to ColorName
        for(ColorName c: colorList) {
        ...
        //convert String color names to ColorName
        for(String color1 :colorNames){
            for(ColorName c: colorList){
                if (c.getName().equals(color1)){
        ...
        int g2 = 0xFF - g;
        int b2 = 0xFF - b;
        //get closest matching color using MSE
        for (ColorName c : availableColors) {
            mse = c.computeMSE(r2, g2, b2);
            if (mse < minMSE) {
                minMSE = mse;
                closestMatch = c;
            }
